...
@doc Markdown.doc"""
   Pythagoras(; name, u1IsHypotenuse)

## Parameters: 

| Name         | Description                         | Units  |   Default value |
| ------------ | ----------------------------------- | ------ | --------------- |
| `u1IsHypotenuse`         | = true, if u1 is the hypotenuse and y is one leg                         | --  |   false |
...
@component function Pythagoras(; name, u1IsHypotenuse=false)
  ...
  append!(__vars, @variables (valid(t)::Bool), [output = true])
  append!(__vars, @variables (y2(t)::Real), [description = "Square of y"])
  ...
  ### Control Structures
  if not(u1IsHypotenuse)
      push!(__eqs, y2 ~ u1 ^ 2 + u2 ^ 2)
      push!(__eqs, y ~ sqrt(y2))
      push!(__eqs, valid ~ true)
  else
      push!(__eqs, y2 ~ u1 ^ 2 - u2 ^ 2)
      push!(__eqs, valid ~ y2 >= 0)
      push!(__eqs, y ~ ifelse(ModelingToolkit.no_if_lifting(y2 >= 0), sqrt(y2), 0))
  end
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(Pythagoras)) = print(io,

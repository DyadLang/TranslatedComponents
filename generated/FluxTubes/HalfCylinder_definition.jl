...
| ------------ | ----------------------------------- | ------ | --------------- |
| `l`         | Axial length orthogonal to flux (=2*pi*r for cylindrical pole and r>>distance between edges)                         | m  |   0.1 |
...
"""
@component function HalfCylinder(; name, l=0.1)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (l::Real = l), [description = "Axial length orthogonal to flux (=2*pi*r for cylindrical pole and r>>distance between edges)"])
  ...
  push!(__eqs, G_m ~ 0.00000125663706212 * 0.26 * l)
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(HalfCylinder)) = print(io,

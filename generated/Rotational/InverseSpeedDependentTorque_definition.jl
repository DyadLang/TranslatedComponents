...
@doc Markdown.doc"""
   InverseSpeedDependentTorque(; name, useSupport, TorqueDirection, tau_nominal, w_nominal, w0)
...
"""
@component function InverseSpeedDependentTorque(; name, useSupport=false, TorqueDirection=true, tau_nominal=nothing, w_nominal=nothing, w0=0.1)
  ...
  append!(__params, @parameters (w0::Real = w0), [description = "Regularization below w0"])
  ...
  if TorqueDirection
      push!(__eqs, tau ~ ifelse(abs(w) < w0, tau_nominal * w_nominal / w0, tau_nominal * w_nominal / abs(w)))
  else
      push!(__eqs, tau ~ ifelse(abs(w) < w0, tau_nominal * w / w0, tau_nominal * w_nominal / w))
  end
...
export InverseSpeedDependentTorque

Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(InverseSpeedDependentTorque)) = print(io,

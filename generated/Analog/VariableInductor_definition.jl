...
@doc Markdown.doc"""
   VariableInductor(; name, UIC, Lmin, IC)
...
| `Lmin`         | Lower bound for variable inductance                         | H  |   Modelica.Constants.eps |
| `IC`         | Initial Value                         | A  |   0 |
 ...
 * `L` - This connector represents a real signal as an input to a component ([`RealInput`](@ref))
...
| `Psi`         |                          | Wb  | 
"""
@component function VariableInductor(; name, UIC=false, Lmin=Modelica.Constants.eps, IC=0)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (Lmin::Real = Lmin), [description = "Lower bound for variable inductance"])
  append!(__params, @parameters (IC::Real = IC), [description = "Initial Value"])

  ### Variables
  __vars = Any[]
  append!(__vars, @variables (L(t)::Real), [input = true])
  ...
  push!(__eqs, i ~ p.i)
  push!(__eqs, Psi ~ identity(max(L, Lmin)) * i)
  push!(__eqs, v ~ D(Psi))
...
export VariableInductor

Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(VariableInductor)) = print(io,

...
"""
@component function Transformer(; name, L1=1, L2=1, M=1)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (L1::Real = L1), [description = "Primary inductance"])
  append!(__params, @parameters (L2::Real = L2), [description = "Secondary inductance"])
  append!(__params, @parameters (M::Real = M), [description = "Coupling inductance"])
  ...
  append!(__vars, @variables (dv(t)::Real), [description = "Difference between voltage drop over primary inductor and voltage drop over secondary inductor"])
  ...
  push!(__eqs, v1 ~ L1 * D(i1) + M * D(i2))
  push!(__eqs, dv ~ (L1 - M) * D(i1) + (M - L2) * D(i2))
  push!(__eqs, v2 ~ v1 - dv)
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(Transformer)) = print(io,

...
@doc Markdown.doc"""
   VariableResistor(; name, useHeatPort, T, T_ref, alpha)
...
| `R_actual`         | Actual resistance = R*(1 + alpha*(T_heatPort - T_ref))                         | Ω  | 
"""
@component function VariableResistor(; name, useHeatPort=false, T=T_ref, T_ref=300.15, alpha=0)
  ...
  append!(__vars, @variables (R_actual(t)::Real), [description = "Actual resistance = R*(1 + alpha*(T_heatPort - T_ref))"])
  ...
  push!(__eqs, i ~ p.i)
  push!(__eqs, R_actual ~ R * (1 + alpha * (T_heatPort - T_ref)))
  push!(__eqs, v ~ R_actual * i)
...
export VariableResistor

Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(VariableResistor)) = print(io,
